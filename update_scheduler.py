...
async def update_session(loop, url, session_id, client_url):
    ...
    async with ms_client.get(MASTER_SERVER_URL+str(session_id)) as get_response,
        ...
        data = yield await get_response.read()
        yield await post_response.write(data)


async def schedule_update(loop, update_period=15.0):
        ...
        if (loop.time() + 1.0) >= end_time:
            ...
            for session in db.sessions.find():
                for url in session['addrs']:
                    yield await update_session(loop=loop,
